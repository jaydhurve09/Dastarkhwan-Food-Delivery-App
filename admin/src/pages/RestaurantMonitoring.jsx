...
const RestaurantMonitoring = () => {
  ...
  const [currentMenuCategoryType, setCurrentMenuCategoryType] = useState(null);
  ...
  return (
    <div style={styles.pageContainer}>
      ...
      {/* --- Modals --- */}
      <Modal isOpen={isMenuModalOpen} onClose={() => { setMenuModalOpen(false); setEditingItem(null); setImagePreview(null); }}>
        ...
        <form onSubmit={handleMenuFormSubmit}>
            ...
            {currentSubCategories.length > 0 && (
             ...
             {/* Price */}
            <div style={styles.formGroup}>
                <label style={styles.label}>Full Price (₹)</label>
                ...
                <label>
                    <input 
                        ...
                        onChange={(e) => {
                            ...
                            if (!e.target.checked) setCurrentSubDishes([]); // Clear sub-dishes if unchecked
                        }}
      ...
      <Modal isOpen={isOfferModalOpen} onClose={() => { setOfferModalOpen(false); setEditingOffer(null); }}>
        {/* Content removed as offers are replaced by orders */}
        <h3>Offer Management Disabled</h3>
      </Modal>

      <Modal isOpen={isCategoryModalOpen} onClose={() => {setCategoryModalOpen(false); setNewCategoryHasSub(false); setNewSubCategoryStr('');}}>
                          ...
                          {menuByCategory[category].map(item => (
                            <tr key={item.id}>
                              ...
                              <td style={styles.td} data-label="Price">
                                {item.price.full && `Full: ₹${item.price.full.toFixed(2)}`}
                              </td>
                              <td style={styles.td} data-label="Available">
                                <label style={styles.toggleSwitch}>
                                  <input type="checkbox" style={styles.toggleInput} checked={item.available} onChange={() => {
                              ...
                              <td style={styles.td} data-label="Actions" className="action-cell-container">
                                <div style={{display: 'flex', flexDirection: 'column', gap: '5px'}}>
                                  <button style={{...styles.button, ...styles.editButton}} onClick={() => { 
                                      setEditingItem(item); 
                                      const cat = categories.find(c => c.name === item.category);
                                      setCurrentSubCategories(cat?.subCategories || []);
                                      setSelectedSubCategory(item.subCategory || '');
                                      setHasSubDishes(!!item.subDishes?.length);
                                      setCurrentSubDishes(item.subDishes || []);
                                      setMenuModalOpen(true); 
                                      setImagePreview(null); 
                                  }}><FaEdit /> Edit</button>
                                  <button style={{...styles.button, ...styles.deleteButton}} onClick={() => handleDeleteMenuItem(item.id)}><FaTrash /> Delete</button>
                                </div>
                              </td>
                            </tr>
                          ))}
